...
client.commands.register(
  ...
  async (ctx) => {
    ...
    //    Voice is server-scoped, and since 0.4.0 a slash command can arrive
    //    from a DM (ctx.serverId null), so narrow before joining.
    if (!ctx.serverId) {
      return void (await ctx.reply('run this in a server, not a DM'));
    }
    conn = await client.joinVoice(ctx.serverId, channelId, { deafened: true });
    ...
    const voiceKey = await client.voiceKeys.acquireVoiceKey(ctx.serverId);
    ...
    session = await VoiceMediaSession.attach({
      ...
      // Not optional in practice: a current backend withholds the microphone
      // until the bot reports its connection up, which attach() does through
      // this reporter before publishing, then waits for the admission.
...
<AccordionGroup>
  ...
  <Accordion title="attach() installs the key before publishing" icon="lock">
    `VoiceMediaSession.attach()` connects, installs the frame key under the identity LiveKit reported, reports the connection up through `connectionReporter`, waits for the microphone admission (up to `admissionTimeoutMs`, default 5 s), and only then publishes. Reverse the key-then-publish order and the frame cryptor is created without a key, so the bot transmits audio nobody can decrypt. Leave the reporter out and there is no wait: a current backend never admits the microphone, so the SFU forwards nothing. See [Microphone admission](/voice/audio#microphone-admission).
