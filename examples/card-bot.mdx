...
function buildStatusCard(run: number, withLogo: boolean) {
  return {
    ...
    // Put everything a reader must actually SEE in `description`. A mobile
    // build older than the 0.4.0 client release drops `fields` silently.
...
<AccordionGroup>
  ...
  <Accordion title="Put the substance in description" icon="align-left">
    <Warning>
      `embed.fields` render on desktop and mobile from the client release that ships with SDK 0.4.0. A mobile build **older than that** drops the rows silently, with no error anywhere, and nothing tells the author.
    </Warning>

    Until your server's members have updated, put anything a reader must see in `description` and treat `fields` as decoration. `description` is plain text, not markdown: v1 renders it literally, which is why the example joins its lines with `\n`.
  </Accordion>
