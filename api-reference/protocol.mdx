...
| `GET_SERVER_COMMANDS` | 626 | Named but never sent by the SDK: a bot declares its own menu and reads nobody's. A human client sends `[626]` for a server's menus, and `[626, botId]` for one bot's menu when it opens a DM with that bot. |
...
On the DM flavor, `SERVER_ID` is the literal `0` and `CHANNEL_ID` carries the dm id. There is no mention array, no reply array, no timestamp and no command data on this frame. From wire version 3 (`CLIENT_WIRE_VERSION`), a backend that honors it also puts the bot on the DM lane's ordinary message broadcast, which carries all of those plus the card slot; the SDK dedupes the two by message id. On an older backend this notify frame is the whole DM signal, which is why a DM message has no `repliedTo` or `createdAt` there.
