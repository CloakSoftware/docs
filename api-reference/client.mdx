...
[`SendOptions`](/api-reference/types#sendoptions) is `{ groupId?, mentions?, replyTo?, poll?, threadId? }`. Pass `threadId` to target a thread of the channel, and `poll` to attach a native poll to the message. See [Threads](/guides/threads) and [Polls](/guides/polls).
...
Poll **creation** rides `send(serverId, channelId, text, { poll })`. The three methods below act on an existing poll and take the message's location, so a `Message`'s own ids are exactly what to pass. There is no vote method: the backend refuses a bot vote by design, so bots create, close, and read polls, and only humans vote. See [Polls](/guides/polls).
...
closePoll(
  ...
  channelId: string,
  messageId: string,
  opts?: { groupId?: string; threadId?: string },
): Promise<void>
```
