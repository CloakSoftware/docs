...
<Note>
  `fetchMessages()` reads a **channel's** history. It cannot address a thread's history yet, so past thread replies are not readable; only live thread messages arrive, on `messageCreate`. See [Known limitations](/production/known-limitations).
</Note>
...
Threads fork out of text channels, and a thread's replies are ordinary messages whose channel is the thread. Send into one with `send(serverId, parentChannelId, text, { threadId })`. See [Threads](/guides/threads) for the model and the thread permissions.
...
<Note>
  Thread enumeration exists, but a thread's **message history** is not addressable yet: [`fetchMessages()`](#fetchmessages) reads the parent channel only. See [Known limitations](/production/known-limitations).
</Note>
