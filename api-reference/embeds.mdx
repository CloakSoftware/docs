...
<ResponseField name="fields" type="BotEmbedField[]">
  Up to 8 rows, in a three-column grid. `inline: false` takes a full row. See the warning below about older mobile builds.
</ResponseField>

<Warning>
  `fields` render on desktop and mobile from the client release that ships with SDK 0.4.0. A mobile build older than that drops the rows silently, with no error anywhere, so until your server's members have updated, keep anything a reader must see in `description`.
</Warning>
