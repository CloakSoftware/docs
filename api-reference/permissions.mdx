...
| `view_voice` | 2 | View voice channels. Joining one is gated separately, see the note below. |
...
<Note>
  `view_voice` does not gate joining a voice channel. The backend's channel-level view check for a voice join runs on a ladder whose server baseline column is `view_text`, so a voice bot declares `view_text` alongside `voice_connect`. See [Joining voice](/voice/joining#permissions).
</Note>
...
| Name | Code | Controls |
| --- | --- | --- |
| `message_send` | 14 | Send messages. Also gates creating a forum post and sending inside one, resolved against the forum channel. |
| `message_manage` | 15 | Manage other members' messages, such as deleting them. Also gates moderating other members' forum posts. |
| `embed_link` | 16 | Post links that render as unfurled previews. Current backends also enforce it for [`sendCard()`](/api-reference/client#sendcard). |
...
Forums have no equivalent names. Creating a post and sending inside one is `message_send` on the forum channel, and moderating other members' posts is `message_manage`. See [Forums](/guides/forums).
