...
`icon` and `miniIcon` are already-hosted image URLs; the SDK does not upload webhook icons for you. The server sanitizes them on write: a non-Cloak URL is rewritten through the image proxy, or dropped when the proxy is unconfigured.
