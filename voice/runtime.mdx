...
| Same, and the engine is fine | `attach()` was given no `connectionReporter`, so a current backend never admitted the microphone (a warning under `debug`) | Pass `connectionReporter: client.voiceConnectionReporter()` to `attach()` |
