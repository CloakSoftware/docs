...
### Direct message pins, and what needs wire version 3

A bot can hold a 1:1 end-to-end encrypted DM with a human, and both directions work. What does not work inside a DM, and what needs a backend that honors wire version 3:

| Call | Behavior in a DM |
| --- | --- |
| `msg.pin()` / `unpin()` | Rejects. The pin operation has no DM lane a bot can observe. |
| `sendCommand()` | Server channels only. |
| `msg.react()` / `unreact()`, `edit()`, `editCard()`, `delete()` | Work on a backend that honors wire version 3. On an older backend a DM message carries no `createdAt` and they reject with a message saying so. |
| `fetchDmMessages()`, `fetchMessages(null, dmId)` | Work on a backend that honors wire version 3. Resolve empty on an older one. |
| Slash commands from the DM `/` picker | Dispatch on a backend that honors wire version 3. A typed `/command` dispatches on any backend. |
| `msg.repliedTo`, `msg.card`, `msg.createdAt` | Populated on a backend that honors wire version 3. `null` on an older backend. |

`reply()` and `channel.send()` work everywhere, and `messageCreate` still fires. See [What a wire version 3 backend adds](/guides/direct-messages#what-a-wire-version-3-backend-adds).
...
### Server voice is microphone-clamped until the bot reports its connection

A current Cloak backend mints a server-voice credential with the microphone withheld and admits it only after the bot reports its media connection up. `VoiceMediaSession.attach()` sends that report through `VoiceMediaOptions.connectionReporter` before it publishes and waits for the admission, up to `admissionTimeoutMs` (default 5 s). A session attached **without** `connectionReporter: client.voiceConnectionReporter()` skips the wait and publishes into a void: the publish succeeds locally, the SFU forwards nothing, and nothing reports it beyond a warning under `debug`. An older single-phase backend is unaffected, which is why a bot that worked there goes silent after a backend upgrade. Pass the reporter on every attach. See [Microphone admission](/voice/audio#microphone-admission).
...
### `fields` and older mobile builds

<Warning>
  Card `fields` render on desktop and mobile from the client release that ships with SDK 0.4.0. A mobile build **older than that** drops the rows silently, with no error anywhere. Until your server's members have updated, keep anything a reader must actually see in `description` and treat `fields` as decoration. The same release is the one that renders component rows, so a member who sees your buttons also sees your fields.
</Warning>
