...
[`fetchThreads()`](/api-reference/client#fetchthreads) enumerates a channel's threads, but a thread's **message history** is not readable: [`fetchMessages()`](/api-reference/client#fetchmessages) addresses the parent channel only. The wire allows it, and the SDK just has not built the paging against the entered-thread cursor yet.

The practical consequence: a bot only sees thread replies that arrive live on `messageCreate`. After a restart, past thread replies cannot be recovered, so state derived from them (a bridge's exact reaction bookkeeping, for example) cannot be rebuilt from history the way channel state can. Expected to change in a later release.
