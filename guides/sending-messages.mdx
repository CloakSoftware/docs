...
<ParamField path="opts.threadId" type="string">
  Send into a thread of the channel. `channelId` is the thread's parent text channel. See [Threads](/guides/threads).
</ParamField>

<ParamField path="opts.postId" type="string">
  Send into a post of a forum channel. `channelId` is the parent forum channel. Mutually exclusive with `threadId`. See [Forums](/guides/forums).
</ParamField>
...
Both methods work on a direct message. There they route into the DM lane instead of `client.send()`, which is why you do not have to branch. `react`, `unreact`, `edit`, `editCard`, and `delete` work there too on a backend that honors wire version 3; `pin` and `unpin` reject on a DM message. See [Direct messages](/guides/direct-messages).
