...
A direct message has no server, so `msg.serverId` is `null` on every DM and `sendCard()` cannot address one. Guard before you use it:
...
`client.sendCardDM(userId, card)` is the DM form: the same card and the same envelope, encrypted under the peer-keyed DM key. `threadId` and `postId` are not valid on it and `groupId` is ignored. See [Direct messages](/guides/direct-messages#cards-in-a-dm).

An inbound card arrives as an ordinary `messageCreate` carrying its `content`, with the decoded card on `msg.card` since 0.4.0: the embed and any component rows, validated on read with the same caps and media rule as on write. See [MessageCard](/api-reference/embeds#messagecard).

### Buttons and selects

A card can also carry up to five rows of buttons and string selects in `components`, and `embed` becomes optional when it does. Clicks arrive on the `interaction` event. See [Buttons and selects](/guides/components).
...
`embed.fields` render on desktop and mobile from the client release that ships with SDK 0.4.0, in the same three-column grid on both, with an `inline: false` row taking the full width.

<Warning>
  A mobile build older than that release drops the rows silently, with no error anywhere and nothing for you to notice. Until your server's members have updated, keep anything a reader must actually see in `description` and treat `fields` as decoration.
</Warning>

```ts
const embed = {
  title: 'Build #4821 passed',
  // Every reader sees this line, on any client build. Keep the substance here.
  description: '42 tests, 0 failures\nmain @ 9f2a1c, 1m 12s',
  fields: [
...
client.on('messageCreate', async (msg) => {
  ...
  if (msg.isDM || !msg.serverId) return; // sendCard() is server-scoped; sendCardDM() is the DM form
